...
async function loadAll() {
  ...
  await loadComponent('nav-placeholder',            'parts/header.html');
  await loadComponent('hero-placeholder',           'parts/hero.html');
  await loadComponent('about-placeholder',          'parts/about.html');
  await loadComponent('skills-placeholder',         'parts/skills.html');
  await loadComponent('projects-placeholder',       'parts/projects.html');
  await loadComponent('research-placeholder',       'parts/research.html');
  await loadComponent('experience-placeholder',     'parts/experience.html');
  await loadComponent('certifications-placeholder', 'parts/certifications.html');
  await loadComponent('workshops-placeholder',      'parts/workshops.html');
  await loadComponent('activities-placeholder',     'parts/activities.html');
  await loadComponent('social-placeholder',         'parts/social.html');
  await loadComponent('contact-placeholder',        'parts/contact.html');
...
function initTyping() {
  ...
  function type() {
    ...
    if (!del) {
      ...
      if (ci === t.length) { del = true; setTimeout(typ, 1900); return; } // BUG HERE
    } else {
      ci--;
      tel.textContent = t.slice(0, ci);
      if (ci === 0) { del = false; ti = (ti + 1) % titles.length; }
    }
    setTimeout(typ, del ? 55 : 88); // BUG HERE
  }
  setTimeout(type, 900);
}
